<template>
  <div>
    <div class="flex items-center flex-shrink-0 p-4 pb-0">
      <!-- Profile Image -->
      <div class="flex w-12 items-start shrink-0">
        <img
          :src="$props.user?.profileImage ?? ''" alt=""
      ...
      <div class="w-full p-2">
        <textarea
          v-model="text"
          class="w-full h-10 text-lg text-gray-900
          placeholder:text-gray-400 bg-transparent border-0
          dark:text-white focus:ring-0"
    ...
    <div class="p-4 pl-16">
      <img
        :src="inputImageUrl" alt="" class="rounded-2xl border"
        :class="twitterBorderColor"
      >
      <input
        ref="imageInput" type="file" hidden
    ...
    <div class="flex p-2 pl-14">
      <div class="flex w-full text-white">
        <!-- Image Icon -->
        <div
          class="p-2 text-blue-400 rounded-full hover:bg-blue-50
      dark:hover:bg-dim-800 cursor-pointer" @click="handleImageClick"
        >
        ...
        <div
          class="p-2 text-blue-400 rounded-full hover:bg-blue-50
      dark:hover:bg-dim-800 cursor-pointer"
        >
          <svg viewBox="0 0 24 24" class="w-5 h-5" fill="currentColor">
            <g>
              <path
...
function handleFormSubmit() {
  emit('onSubmit', {
    text: text.value,
    mediaFiles: selectedFile.value ? [selectedFile.value] : [],
  });
}
